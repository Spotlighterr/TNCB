...
- [x] **Bước 1**: Cập nhật lại các biến CSS Tokens trong `src/styles/variables.css`:
...
- [x] **Bước 2**: Rà soát các component trên giao diện (Header, Button, Thẻ phòng trọ, Bento Stats) để đảm bảo không bị sót màu ngọc lục bảo cũ hoặc màu tím AI rập khuôn.
- [x] **Bước 3**: Tinh chỉnh độ tương phản màu chữ trên nền đỏ thương hiệu để đạt chuẩn dễ đọc WCAG AA.
- [x] **Bước 4**: Rebuild và kiểm thử toàn bộ giao diện xem tông màu đỏ mới có hài hòa và mang cảm giác chuyên nghiệp của hệ sinh thái FTU.

---

## 🎨 12. Tái thiết kế Giao diện Trang chủ và Bộ lọc theo phong cách Apple & FTUGate

Bản thiết kế mới của trang chủ kết hợp triết lý thiết kế tối giản của Apple và màu sắc nhận diện FTUGate:

- [x] **Bước 1**: Di chuyển form tìm kiếm từ banner mở đầu xuống bên trong thẻ Bento "Hệ sinh thái số" để tối giản hóa banner.
- [x] **Bước 2**: Gộp 3 tab bản tin (Cộng đồng FindX, Hoạt động nổi bật, Phòng trọ kiểu mẫu) thành một Banner Carousel trượt thống nhất với tiêu đề Metallic và Glowing Siri-style.
- [x] **Bước 3**: Tích hợp bộ lọc diện tích tối thiểu và tối đa ($m^2$) vào trang tìm kiếm `Search.jsx`.
- [x] **Bước 4**: Điều chỉnh kích thước các thẻ thống kê Bento to hơn để lấp đầy khoảng trống.
- [x] **Bước 5**: Tối ưu hóa chiều cao slide và các khối văn bản để tránh hiện tượng giật hay lệch khung hình khi chuyển slide.
- [x] **Bước 6**: Đồng bộ tiêu đề trang chủ "Tìm phòng trọ dễ dàng" theo hiệu ứng metallic và gradient sặc sỡ và làm trắng nền banner.
